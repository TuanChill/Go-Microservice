...
import (
	...
	_ "go_template/docs/swagger"
	"go_template/global"
	"go_template/internal/controllers"
	controller "go_template/internal/controllers"
	"go_template/internal/middlewares"
	"go_template/response"
	third_party "go_template/third_party/telegram"
...
func NotFound() gin.HandlerFunc {
	return func(c *gin.Context) {
		response.NotFoundError(c, response.ErrCodeResourceNotFound)
	}
}
...
func ServiceUnavailable() gin.HandlerFunc {
	return func(c *gin.Context) {
		c.Next()
		if len(c.Errors) > 0 {
			response.ServiceUnavailable(c, response.ErrCodeServiceUnavailable)
		}
	}
